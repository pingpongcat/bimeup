...
BloomUpPipeline::BloomUpPipeline(const Device& device,
...
BloomUpPipeline::~BloomUpPipeline() = default;

void BloomUpPipeline::Bind(VkCommandBuffer cmd) const {
    m_pipeline->Bind(cmd);
}

VkPipeline BloomUpPipeline::GetPipeline() const {
    return m_pipeline->GetPipeline();
}

VkPipelineLayout BloomUpPipeline::GetLayout() const {
