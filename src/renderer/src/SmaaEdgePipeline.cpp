...
SmaaEdgePipeline::SmaaEdgePipeline(const Device& device,
...
SmaaEdgePipeline::~SmaaEdgePipeline() = default;

void SmaaEdgePipeline::Bind(VkCommandBuffer cmd) const {
    m_pipeline->Bind(cmd);
}

VkPipeline SmaaEdgePipeline::GetPipeline() const {
    return m_pipeline->GetPipeline();
}

VkPipelineLayout SmaaEdgePipeline::GetLayout() const {
