...
SsaoPipeline::SsaoPipeline(const Device& device,
    ...
    if (vkCreatePipelineLayout(m_device, &layoutInfo, nullptr, &m_layout) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create SsaoPipeline layout");
    }
    ...
    if (vkCreateComputePipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr,
        ...
        throw std::runtime_error("Failed to create SsaoPipeline");
    }
}
...
void SsaoPipeline::Bind(VkCommandBuffer cmd) const {
