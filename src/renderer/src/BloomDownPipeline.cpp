...
BloomDownPipeline::BloomDownPipeline(const Device& device,
    ...
    // Downsample overwrites the target mip — the composite strategy (how the
    // later upsample blends back in) is an RP.10c wiring concern.
...
BloomDownPipeline::~BloomDownPipeline() = default;

void BloomDownPipeline::Bind(VkCommandBuffer cmd) const {
    m_pipeline->Bind(cmd);
}

VkPipeline BloomDownPipeline::GetPipeline() const {
    return m_pipeline->GetPipeline();
}

VkPipelineLayout BloomDownPipeline::GetLayout() const {
