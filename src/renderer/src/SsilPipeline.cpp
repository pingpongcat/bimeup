...
SsilPipeline::SsilPipeline(const Device& device,
    ...
    if (vkCreatePipelineLayout(m_device, &layoutInfo, nullptr, &m_layout) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create SsilPipeline layout");
    }
    ...
    if (vkCreateComputePipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr,
        ...
        throw std::runtime_error("Failed to create SsilPipeline");
    }
}

SsilPipeline::~SsilPipeline() {
...
void SsilPipeline::Bind(VkCommandBuffer cmd) const {
