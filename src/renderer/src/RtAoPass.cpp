...
// AO-pass descriptor set: TLAS (0), depth image (1), AO storage image (2).
// Layout is identical to `RtShadowPass`'s — bindings are raygen-only
// (miss + skipped CH never read them).
VkDescriptorSetLayout CreateLayout(VkDevice dev) {
    ...
    if (vkCreateDescriptorSetLayout(dev, &info, nullptr, &layout) != VK_SUCCESS) {
        throw std::runtime_error("RtAoPass: vkCreateDescriptorSetLayout failed");
    }
    return layout;
}
...
RtAoPass::RtAoPass(const Device& device) : m_device(device) {
...
void RtAoPass::CreateAoImage(uint32_t width, uint32_t height) {
    ...
    if (vmaCreateImage(m_device.GetAllocator(), &ii, &ai, &m_image, &m_allocation,
                       nullptr) != VK_SUCCESS) {
        throw std::runtime_error("RtAoPass: vmaCreateImage failed");
    }
    ...
    if (vkCreateImageView(m_device.GetDevice(), &vi, nullptr, &m_imageView) != VK_SUCCESS) {
        throw std::runtime_error("RtAoPass: vkCreateImageView failed");
    }
    ...
    if (vkCreateSampler(m_device.GetDevice(), &si, nullptr, &m_sampler) != VK_SUCCESS) {
        throw std::runtime_error("RtAoPass: vkCreateSampler failed");
    }
...
void RtAoPass::CreateDescriptor() {
    VkDevice dev = m_device.GetDevice();
...
bool RtAoPass::Build(uint32_t width, uint32_t height, const std::string& shaderDir) {
    if (!m_device.HasRayTracing()) {
        return false;
    }
    if (width == 0 || height == 0) {
    ...
    Reset();
    CreateAoImage(width, height);
    CreateDescriptor();

    m_pipeline = std::make_unique<RayTracingPipeline>(m_device);

    RayTracingPipelineSettings s;
    s.raygenPath = shaderDir + "/rtao.rgen.spv";
    s.missPath = shaderDir + "/rtao.rmiss.spv";
    // AO raygen sets `gl_RayFlagsSkipClosestHitShaderEXT` (only presence
    // of a hit matters — binary visibility), so we re-use the Stage-9.3
    // empty stub just to satisfy the RT-pipeline group layout.
