...
SmaaBlendPipeline::SmaaBlendPipeline(const Device& device,
...
SmaaBlendPipeline::~SmaaBlendPipeline() = default;

void SmaaBlendPipeline::Bind(VkCommandBuffer cmd) const {
    m_pipeline->Bind(cmd);
}

VkPipeline SmaaBlendPipeline::GetPipeline() const {
    return m_pipeline->GetPipeline();
}

VkPipelineLayout SmaaBlendPipeline::GetLayout() const {
