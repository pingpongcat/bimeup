...
## Current Task: 9.Q.3 — `basic.frag` gains `#extension GL_EXT_ray_query : require` + a `uint useRayQueryShadow` push-constant bit + a ray-query branch replacing the PCF shadow-map sample when the flag is on. Glass transmission (RP.18 tint path) stays active.
