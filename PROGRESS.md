...
## Current Task: 9.8.c.2 RtIndoorCompositePipeline + rt_indoor_composite.comp. RP.17.6 edge-snap permanently deferred (classical-raster nice-to-have, Stage 9.6 subsumes it via RT).
...
- [ ] 9.8 Hybrid composite — per-contribution raster/RT routing; Rasterised mode stays bit-compatible with pre-Stage-9 output. Sub-split 2026-04-20: `basic.frag` is forward-shaded and bakes sun + PCF + transmission + fill; RT dispatches land after the main pass → AO can composite cleanly in `tonemap.frag`, shadow + indoor need deferred-contribution composite passes.
  ...
  - [ ] 9.8.c RT indoor-fill routing — same structure as 9.8.b for the fill term. Sub-sub-split 2026-04-20.
    ...
    - [ ] 9.8.c.2 `renderer::RtIndoorCompositePipeline` + `rt_indoor_composite.comp` — additive fill * NdotL * RT wall-occlusion into the HDR storage image.
    - [ ] 9.8.c.3 RenderLoop wire — lazy-build alongside `m_rtIndoorPass`; `SetRtIndoorCompositeInputs` + `DispatchRtIndoorComposite` slot.
    - [ ] 9.8.c.4 Stage gate — full `ctest -j$(nproc) --output-on-failure` ✓ (visual check deferred to 9.8.d with main.cpp plumbing).
