...
## Current Task: 9.7 Indoor-light sampling — overhead fill respects walls when `indoorLightsEnabled` (RT modes only). RP.17.6 edge-snap permanently deferred (classical-raster nice-to-have, Stage 9.6 subsumes it via RT).
...
- [ ] 9.7 Indoor-light sampling — overhead fill respects walls when `indoorLightsEnabled` (RT modes only)
  ...
  - [ ] 9.7.b `RenderLoop` wire — `SetRtIndoorInputs(fillDirWorld, enabled)` + `GetRtIndoorVisibilityView()` accessor; build/dispatch alongside shadow + AO in HybridRt; skip dispatch when `enabled=false`.
