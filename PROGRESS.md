...
## Current Task: 9.8.e NRD integration — denoiser for production-quality RT outputs. RP.17.6 edge-snap permanently deferred (classical-raster nice-to-have, Stage 9.6 subsumes it via RT).
...
- [ ] 9.8 Hybrid composite — per-contribution raster/RT routing; Rasterised mode stays bit-compatible with pre-Stage-9 output. Sub-split 2026-04-20: `basic.frag` is forward-shaded and bakes sun + PCF + transmission + fill; RT dispatches land after the main pass → AO can composite cleanly in `tonemap.frag`, shadow + indoor need deferred-contribution composite passes.
  ...
  - [ ] 9.8.e NRD integration — NVIDIA Real-time Denoiser (open-source, cross-vendor) for production-quality RT output. Sub-split: add NRD as git submodule to `external/`, implement motion-vector generation in geometry pass (velocity G-buffer), wire SIGMA denoiser for RT shadow visibility (1-3ms @ 1080p), wire RELAX denoiser for RTAO + indoor-fill visibility (1-2ms each). Transforms the single-sample-per-pixel noisy RT outputs into smooth, AAA-quality results. NRD requires depth + normals + motion vectors + per-frame history buffers; motion vectors feed TAA-style reprojection for temporal stability when camera moves. Plan: `docs/plan/stage_9_raytracing.md` → "9.8.e".
