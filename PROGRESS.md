...
## Current Task: 9.Q.4 — `RenderQualityPanel` gains a three-radio selector (`Rasterised` / `Ray query` / `Ray tracing (disabled)`). `main.cpp` maps it onto a new `RenderLoop::RenderMode::RayQuery` + pushes `useRayQueryShadow = 1` per draw when active + writes the TLAS into the main descriptor set via `SceneUploader::WriteTlasToDescriptor`. Adds binding 5 (TLAS) to the main descriptor-set layout.
