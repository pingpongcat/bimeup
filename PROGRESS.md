...
## Current Task: 9.Q.5 — stage gate (visual: ray-query shadows match/exceed PCF on the sample scene; full `ctest -j$(nproc) --output-on-failure` green).
...
- [x] 9.Q.2 TLAS descriptor binding plumbed into the main descriptor set layout (so `basic.frag` can access it). Binding is optional — raster mode leaves it null. `DescriptorSet::UpdateAccelerationStructure(binding, tlas)` writes a `VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR` descriptor via a chained `VkWriteDescriptorSetAccelerationStructureKHR` (caller must pass a valid handle); `SceneUploader::WriteTlasToDescriptor(set, binding, tlas)` is the integration entry point — no-op when `tlas.IsValid()` is false (raster mode / RT-unavailable / scene with no instances). The actual binding slot in the main layout + `basic.frag` declaration are 9.Q.3 work; this task just lands the API + tests. Tests: 2 in `DescriptorSetTest` (`LayoutAcceptsAccelerationStructureBinding` + `UpdateWithAccelerationStructure`, both `GTEST_SKIP` on devices without AS) + 2 in `SceneUploaderTest` (`WriteTlasToDescriptorIsNoOpWhenTlasInvalid` + `WriteTlasToDescriptorWritesValidTlas`, same skip gate). Renderer suite 273/273 ✓; core suite 45/45 ✓.
- [x] 9.Q.3 `basic.frag` bumped to `#version 460` + `#extension GL_EXT_ray_query : require` + new TLAS binding (set 0, binding 5) + new push-constant member `uint useRayQueryShadow` @ offset 76 (range now 4 uints / 16 bytes). New `rayQueryShadow(worldPos, n)` helper traces a single shadow ray with `OpaqueEXT | TerminateOnFirstHit` flags using the sun direction from `LightingUBO.keyDirectionIntensity`; surface-normal epsilon (0.001) prevents self-intersection. The sun-block visibility line picks ray-query vs PCF via `(push.useRayQueryShadow == 1U) ? rayQueryShadow(...) : pcfShadow(...)` — glass-transmission tint composes on top of either via the unchanged RP.18 path. `CompileShaders.cmake` learned a new `RAY_QUERY_RASTER_NAMES` bucket (currently just `basic.frag`) that gets pulled out of the default raster glob and compiled with `--target-env=vulkan1.2 --target-spv=spv1.4` like the RT shaders. `main.cpp` + `CubeRenderTest.cpp` push-constant range bumped to 4 uints; opaque + transparent draw-call `fragPush` arrays widened to 4 elements with `useRayQueryShadow = 0` for now (the panel→`RenderMode::RayQuery` wire that flips it to 1 lands in 9.Q.4). The shader's `tlas` binding stays declared even when the descriptor-set layout lacks slot 5 — modern validators don't complain about a declared-but-unaccessed binding when the push-constant gate proves the access is dead. Tests: extended `BasicShaderTest.FragmentShaderDeclaresTransparentBitPushConstant` to assert exactly four 32-bit uint members at offsets 64/68/72/76; new `BasicShaderTest.FragmentShaderDeclaresRayQueryCapability` walks SPIR-V `OpCapability` for `RayQueryKHR` (4472). 273/273 renderer ✓; 45/45 core ✓; 5/5 Application+RenderLoop ✓.
- [x] 9.Q.4 `RenderQualityPanel` — three-radio selector pivoted from 9.8.d's `Hybrid RT` / `Path Traced` to `Ray query` / `Ray tracing` (in `bimeup::ui::RenderMode`). `Ray query` greyed out + tooltipped when `Device::HasRayQuery()` is false (gated by the new `rayQueryAvailable` settings field, replacing `rayTracingAvailable`); `Ray tracing` permanently disabled with a "future work — Stage 9.RT" tooltip. `RenderLoop::RenderMode::RayQuery = 2` joins `Rasterised` / `HybridRt` (no per-frame RT dispatch needed — the trace happens inline in `basic.frag`; `SetRenderMode` still tears down any HybridRt resources via the existing `else` branch). `main.cpp`: panel `mode` field gates `dsLayout` binding 5 (TLAS, `VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR`) + matching pool size on `device.HasRayQuery()`; TLAS-build guard widened to `HasRayTracing() || HasRayQuery()` (both modes share the AS stack); `SceneUploader::WriteTlasToDescriptor(descriptorSet, 5, rtTlas)` runs after the TLAS build so basic.frag's binding 5 has a valid handle; render-mode sync maps `panelMode == RayQuery && HasRayQuery()` → `RenderLoop::RenderMode::RayQuery`; per-opaque-draw `fragPush[3] = (currentMode == RayQuery) ? 1U : 0U` flips `useRayQueryShadow`. Transparent draws stay on raster path (all flags 0). Tests: `RenderQualityPanelTest` updated — `DefaultRayTracingAvailabilityIsFalse` → `DefaultRayQueryAvailabilityIsFalse`; new `RenderModeEnumHasRayQueryAndRayTracing` compile-time guard. 273/273 renderer ✓; 145/145 ui ✓; 45/45 core ✓; 4/4 Application ✓.
