...
## Current Stage: Stage 9 — Ray Tracing (pivot 2026-04-23: ray_query over RT pipelines)
## Current Task: Next session — rewrite Stage 9 on top of `VK_KHR_ray_query` (inline ray tracing in `basic.frag` / XeGTAO compute). Retires 9.3 through 9.8.c below; BLAS/TLAS machinery (9.1, 9.2) stays. See `docs/plan/stage_9_ray_tracing.md` (to be updated next session).

> **Pivot 2026-04-23**: hybrid-RT pipelines (separate raygen/miss/hit shader pipelines + dedicated descriptor pools + composite compute passes) are overkill for this viewer's needs (single-ray visibility queries: sun shadow, RTAO, window transmission, indoor-fill occlusion — no recursion, no multi-bounce). Persistent `VUID-vkUpdateDescriptorSets-None-03047` on per-frame descriptor rewires (RtShadow/Ao/Indoor pass + the two composite sets) confirmed the per-frame plumbing is too fragile for the payoff. Ray queries collapse it all into inline `rayQueryInitializeEXT`/`ProceedEXT` calls in the existing `basic.frag` + `ssao_xegtao.comp`: no SBT, no dedicated RT descriptor sets, no composite pass, no per-frame update dance. The hybrid-RT test fix series from this session was reverted. 9.9 path tracer (if ever pursued) remains free to have its own RT pipeline — orthogonal to the visibility-query rewrite.
...
> **RETIRED 2026-04-23 — pivoted to `VK_KHR_ray_query`.** Tasks 9.3 through 9.8.d below are preserved as historical record but represent a superseded approach. The RT-pipeline implementation (dedicated raygen/miss/hit pipelines + separate descriptor pools + per-contribution composite compute passes) hit a persistent `VUID-vkUpdateDescriptorSets-None-03047` with per-frame descriptor rewires across `RtShadow/Ao/IndoorPass` + the sun/indoor composite sets. For this viewer's actual RT needs (single-ray visibility queries with no recursion), inline ray queries in `basic.frag` / `ssao_xegtao.comp` collapse the entire edifice — no SBT, no dedicated RT descriptor sets, no composite passes, no per-frame update dance. 9.1 + 9.2 (BLAS/TLAS machinery) stay. Next session: rewrite 9.3+ as "Stage 9 — Ray queries (additive)" and update `docs/plan/stage_9_ray_tracing.md`. 9.9 path tracer is orthogonal — if ever built, it can have its own RT pipeline.
