...
- [ ] 7.5 Section view (BRep approach — see `docs/section_cap_next_session.md`)
  ...
  - [ ] 7.5k Per-element capping (next session) — debug on `../sample.ifc` (simpler repro than `sample_house.ifc`: one IfcWall + one IfcSlab; with an X-axis clip plane + Section fill enabled, only the IfcWall caps, the IfcSlab underneath does not — see session log / screenshot). Root cause in 7.5i: `BuildSectionCapVertices` slices the whole (batched) `SceneMesh` as one segment bag, so only the loop that stitches cleanest gets drawn. A per-element-color attempt (filter segments by `triangleOwners[t] == nodeId`, tint with darkened vertex color) passed synthetic tests but produced zero caps on the real IFC — likely because per-element segment subsets don't close into polygons and `StitchSegments` drops open polylines. Next session on `../sample.ifc`: inspect segment counts per node, relax stitch epsilon / keep open polylines as fallback caps, add per-element color.
