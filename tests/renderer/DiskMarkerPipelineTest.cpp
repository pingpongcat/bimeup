...
using bimeup::renderer::DiskMarkerPipeline;
...
TEST_F(DiskMarkerPipelineTest, DiskMarkerShadersCompiledToSpirv) {
    std::string shaderDir = BIMEUP_SHADER_DIR;
    EXPECT_TRUE(std::filesystem::exists(shaderDir + "/disk_marker.vert.spv"));
    EXPECT_TRUE(std::filesystem::exists(shaderDir + "/disk_marker.frag.spv"));
}
