...
using bimeup::renderer::RtAoPass;
using bimeup::renderer::TlasInstance;
using bimeup::renderer::TopLevelAS;
using bimeup::renderer::VulkanContext;

// Stage 9.5.a — RT ambient-occlusion pass. Same RT-contract as 9.4.a:
// strict no-op on non-RT devices; GPU-path tests `GTEST_SKIP` when the
// capability probe says no.
class RtAoPassTest : public ::testing::Test {
protected:
...
std::unique_ptr<VulkanContext> RtAoPassTest::s_context;
std::unique_ptr<Device> RtAoPassTest::s_device;

TEST_F(RtAoPassTest, ConstructSucceedsOnAnyDevice) {
    RtAoPass pass(*m_device);
    EXPECT_FALSE(pass.IsValid());
    EXPECT_EQ(pass.GetAoImage(), VK_NULL_HANDLE);
    EXPECT_EQ(pass.GetAoImageView(), VK_NULL_HANDLE);
    EXPECT_EQ(pass.GetWidth(), 0u);
    EXPECT_EQ(pass.GetHeight(), 0u);
}

TEST_F(RtAoPassTest, BuildNoOpWhenRayTracingUnavailable) {
...
TEST_F(RtAoPassTest, BuildAllocatesAoResourcesOnRtDevice) {
    if (!m_device->HasRayTracing()) {
    ...
    EXPECT_NE(pass.GetAoImage(), VK_NULL_HANDLE);
    EXPECT_NE(pass.GetAoImageView(), VK_NULL_HANDLE);
    EXPECT_NE(pass.GetAoSampler(), VK_NULL_HANDLE);
    EXPECT_NE(pass.GetDescriptorSetLayout(), VK_NULL_HANDLE);
    EXPECT_EQ(pass.GetWidth(), 64u);
    EXPECT_EQ(pass.GetHeight(), 48u);
}

TEST_F(RtAoPassTest, RebuildAtNewExtentReplacesImage) {
    if (!m_device->HasRayTracing()) {
        GTEST_SKIP() << "Device does not advertise RT — build-path skipped";
    }
    RtAoPass pass(*m_device);
    ASSERT_TRUE(pass.Build(32u, 32u, ShaderDir()));
    const VkImage first = pass.GetAoImage();
    ASSERT_NE(first, VK_NULL_HANDLE);

    ASSERT_TRUE(pass.Build(64u, 64u, ShaderDir()));
    EXPECT_NE(pass.GetAoImage(), first);
    EXPECT_EQ(pass.GetWidth(), 64u);
    EXPECT_EQ(pass.GetHeight(), 64u);
}

TEST_F(RtAoPassTest, PushConstantsSize144Bytes) {
    // mat4 invViewProj (64) + mat4 invView (64) + uvec2 extent (8) +
    // float radius (4) + uint frameIndex (4) = 144 B. Pinned here to
    // catch accidental field drift — above the 128 B Vulkan-1.2
    // minimum-guarantee, but RT-capable GPUs advertise ≥256 B.
    EXPECT_EQ(sizeof(RtAoPass::PushConstants), 144U);
}

TEST_F(RtAoPassTest, DispatchRecordsAndSubmitsCleanlyOnRtDevice) {
    if (!m_device->HasRayTracing()) {
    ...
    // 1-instance TLAS around a single triangle BLAS — gives the raygen
    // real geometry to trace rays against.
    ...
    // Small depth image cleared to 0.5 so every raygen thread reconstructs
    // a valid world-space position + normal and fires a real AO ray.
